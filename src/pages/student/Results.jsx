...
const Results = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        // Get the authenticated access token through the centralized
        // storage service instead of reading localStorage directly.
        ...
        if (response.status === 404) {
          throw new Error(
            "Results endpoint not found. Check backend routes."
          );
        }
        ...
        {state.results.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-center px-4">
            ...
            <p className="text-sm text-gray-500 mt-2 max-w-sm">
              Complete an assigned assessment to see your performance
              metrics appear here.
            </p>
          ...
          <div className="divide-y divide-gray-50">
            {state.results.map((result) => {
              ...
              return (
                <div
                  key={result._id}
                  className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 hover:bg-gray-50/50 transition-colors group"
                >
                  {/* Quiz Info */}
